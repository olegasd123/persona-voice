...
REM   run-cuda.bat            auto-detect the GPU (RTX 5090 / RTX 4080)
REM   run-cuda.bat 5090       force the 32 GB unquantized profile
REM   run-cuda.bat 4080       force the 16 GB AWQ profile
REM
...
if "%~1"=="" (
    ...
    powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0run-cuda.ps1" -Gpu %~1
)
endlocal
