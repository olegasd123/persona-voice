...
def f5_finetune_command(cfg: VoiceTrainConfig) -> list[str]:
    ...
    command = [
        "f5-tts_finetune-cli",
        "--exp_name", cfg.base_model,
        "--dataset_name", cfg.resolved_dataset_name,
        "--learning_rate", str(cfg.learning_rate),
        "--batch_size_per_gpu", str(cfg.batch_size),
        "--grad_accumulation_steps", str(cfg.grad_accum),
        "--epochs", str(int(cfg.epochs)),
        "--num_warmup_updates", str(cfg.warmup_updates),
        "--save_per_updates", str(cfg.save_every),
